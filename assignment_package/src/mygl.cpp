...
MyGL::MyGL(QWidget *parent)
    : GLWidget277(parent),
      ...
      recursionLimit(5),
      numPhotons(300000), photonSearchRadius(0.01f),
...
void MyGL::RenderScene()
{
    ...
    if(integratorType == PHOTON_MAPPING)
    {
        ...
        for(int i = 0; i < 8; ++i)
        {
            ...
            int seed = i;
            PhotonMapper* pm = new PhotonMapper(threadPhotonCount, &((*photonSets))[i], &((*causticSets))[i],
                                                &scene, sampler->Clone(seed), recursionLimit);
        ...
        for(std::vector<Photon>& cs : *causticSets)
        {
            allCaustics->insert(allCaustics->end(), cs.begin(), cs.end());
        }
        delete causticSets;
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case NAIVE_LIGHTING:
                ...
                break;
            case PHOTON_MAPPING:
                rt = new PhotonMapper(tileBounds, &scene, sampler->Clone(seed), recursionLimit, photonSearchRadius, &kdTree, &causticTree);
                break;
            }
