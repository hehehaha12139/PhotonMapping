...
SampledSpectrum SampledSpectrum::X = SampledSpectrum();
SampledSpectrum SampledSpectrum::Y = SampledSpectrum();
SampledSpectrum SampledSpectrum::Z = SampledSpectrum();
float SampledSpectrum::yint = 0.0f;
...
Spectrum::Spectrum(int _nSamples, float v):
    nSamples(_nSamples)
{
...
Spectrum operator*(const float coef, const Spectrum &s)
{
    Spectrum ret = Spectrum(s.nSamples);
    for(int i = 0; i < s.nSamples; i++)
    {
        ret.c.at(i) = coef * s.c.at(i);
    }
    return ret;
}
...
Spectrum operator/(const float coef, const Spectrum &s)
{
    ...
    for(int i = 0; i < s.nSamples; i++)
    {
        ret.c.at(i) = s.c.at(i) / (coef + FLT_EPSILON);
    }
    return ret;
}
...
Spectrum Spectrum::operator/(const Spectrum &s2) const
{
    ...
    for(int i = 0; i < nSamples; ++i)
    {
        ret.c.at(i) /= (s2.c.at(i) + FLT_EPSILON);
    }
    return ret;
}
...
Spectrum Sqrt(const Spectrum &s)
{
    Spectrum ret = Spectrum(s.nSamples);
    for(int i = 0; i < s.nSamples; i++)
        ret.c.at(i) = sqrtf(s.c.at(i));
    return ret;
}

Spectrum Spectrum::Lerp(float t, const Spectrum &s1, Spectrum &s2)
{
...
{
    Spectrum ret = Spectrum(nSamples);
    for(int i = 0; i < nSamples; i++)
    {
        ret.c.at(i) = glm::clamp(c.at(i), low, high);
    }
    return ret;
}
...
SampledSpectrum SampledSpectrum::FromSampled(const float *lambda, const float *v, int n)
{
    ...
    for(int i = 0; i < nSpectralSamples; i++)
    {
        ...
        float lambda0 = (1.0f - t0) * sampledLambdaStart + t0 * sampledLambdaEnd;
        float t1 = float(i + 1) / float(nSpectralSamples);
        float lambda1 = (1.0f - t1) * sampledLambdaStart + t1 * sampledLambdaEnd;
        r.c.at(i) = AverageSpectrumSamples(lambda, v, n, lambda0, lambda1);
    }
    return r;
}

bool SampledSpectrum::SpectrumSampledSorted(const float *lambda, const float *v, int n)
{
    float cur = FLT_MIN;
    for(int i = 0; i < n; i++)
    {
...
void SampledSpectrum::SortSpectrumSamples(std::vector<float> &lambda, std::vector<float> &v, int n)
{
    quickSortLambda(lambda, v, 0, n - 1);
}

void SampledSpectrum::quickSortLambda(std::vector<float> &lambda, std::vector<float> &v, int l, int r)
{
    if(l < r)
    {
        int i = l, j = r, x = lambda.at(l);
        while (i < j)
        {
            while(i < j && lambda.at(j) > x)
            {
                j--;
            }
            if(i < j)
            {
                // Detect a value smaller than pivot
                swap(lambda, i + 1, j);
                swap(v, i + 1, j);
            }
            while(i < j && lambda[i] < x)
            {
                i++;
            }
            if(i < j)
        ...
        lambda.at(i) = x;
        quickSortLambda(lambda, v, l, i - 1);
        quickSortLambda(lambda, v, i + 1, r);
    }
}

void swap(std::vector<float> &vec, int ori, int dest)
{
    float value = vec.at(dest);
    vec.at(dest) = vec.at(ori);
    vec.at(ori) = value;
}

float SampledSpectrum::AverageSpectrumSamples(const float *lambda, const float *vals, int n,
                                              float lambdaStart, float lambdaEnd)
{
    ...
    {
        float segStart = fmax(lambdaStart, lambda[i]);
        float segEnd = fmin(lambdaEnd, lambda[i + 1]);
        float t0 = (segStart - lambda[i]) / (lambda[i + 1] - lambda[i]);
        float t1 = (segEnd - lambda[i]) / (lambda[i + 1] - lambda[i]);
        float lerp1 = (1.0f - t0) * vals[i] + t0 * vals[i + 1];
        float lerp2 = (1.0f - t1) * vals[i] + t1 * vals[i + 1];
        sum += 0.5f * (lerp1 + lerp2) * (segEnd - segStart);
    }
